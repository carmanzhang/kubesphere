...
const (
	...
	NodeAddressRule             = `count(label_replace(label_replace(node_uname_info, "internal_ip", "$1", "instance", "(.+):(.+)"), "node", "$1", "nodename", "(.*)")) by (node, internal_ip)`
...
const (
	...
	NodeAddressLabelKey = "internal_ip"
)
